...
public class GetReviewQueueToolHandler implements ToolHandler {
  ...
  public String description() {
    return "Answers: which obligations still need a human decision, ordered descending by"
        ...
        + " ({id, contractId, product, displayName, identityType, cadence,"
        + " annualCostEstimate, txnCount, lastSeen}); pass verbose=true for the full"
        + " evidence/recurring blob. product names the contract's product when one SEPA"
        + " mandate bundles several (one contract per product, each with its own cost and"
        + " cadence); the key is absent for a mandate whose creditor has no product rule."
