...
public class ObligationsRegisterToolHandler implements ToolHandler {
  ...
  public String description() {
    return "Answers: what recurring obligations (insurance, subscriptions, contracts) do I have"
        ...
        + " its OWN contract -- never the counterparty's combined debit. A row carries"
        + " product when one SEPA mandate bundles several products (one contract per"
        + " product, each with its own annual cost); the key is absent for a mandate whose"
        + " creditor has no product rule. totalAnnualCost is"
