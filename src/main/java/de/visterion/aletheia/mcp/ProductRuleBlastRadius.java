 ...
 *
 * @param candidateRoots raw roots of the creditor the rule would actually act on: roots a human
 *     already split or re-attributed are excluded here exactly as the resolver skips them.
 *     Deliberately <b>not</b> the same denominator as {@code list_product_rules}' {@code
 *     rootsVisited}, which counts a skipped root as visited, so {@code rootsVisited >=
 *     candidateRoots} with the difference being the number of human-decided bookings
...
public record ProductRuleBlastRadius(
    int candidateRoots, int bookingsMatched, int positionsParsed, int sumMismatches) {}
