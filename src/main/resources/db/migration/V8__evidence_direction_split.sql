-- Direction-split spend columns for v_counterparty_evidence (spec M2).
--
-- spend_last_365d (V5) sums amount regardless of direction; amount is always positive, so a
-- counterparty with CRDT refunds gets an inflated cost estimate. This migration adds three
-- FILTERed columns so callers (AnnualCost) can use debit-only spend instead:
--   debit_last_365d  -- DBIT-only, last 365 days
--   credit_last_365d -- CRDT-only, last 365 days
--   credit_total      -- all CRDT, all time
--
-- The rest of the view body is reproduced verbatim from V5__evidence_view.sql:14-91.
DROP VIEW v_counterparty_evidence;
