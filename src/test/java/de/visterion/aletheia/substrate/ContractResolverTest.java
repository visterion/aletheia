 ...
 * {@link ContractResolver#resolve()} must not fail application boot when it happens via {@link
 * ContractResolver#run(org.springframework.boot.ApplicationArguments)}, but must still propagate
 * when {@code resolve()} is called directly (e.g. from the ingest endpoint path).
 *
...
class ContractResolverTest {
  ...
  void resolveThrowsWhenDatabaseFails() {
    ContractResolver resolver = new ContractResolver(failingDb());
